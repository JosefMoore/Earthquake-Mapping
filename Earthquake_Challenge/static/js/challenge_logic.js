...
// 4. Use the same style as the earthquake data.
  function styleInfoMajor(feature){
    return{
      opacity: 0.8,
      fillOpacity: 1,
      fillColor: getColorMagnitude(feature.properties.mag),
      color: '#000000',
      radius: getRadiusMagnitude(feature.properties.mag),
      stroke: true,
      weight: 0.5
    }
  }
...
function getColorMagnitude(magnitude) {
  return magnitude > 6 ? 'DarkRed' :
         magnitude > 5 ? '#FFFF00' :
                          '#6600FF';
}

// 6. Use the function that determines the radius of the earthquake marker based on its magnitude.
function getRadiusMagnitude(magnitude) {
  if (magnitude === 0) {
    return 1;
  }
  return magnitude * 5;
